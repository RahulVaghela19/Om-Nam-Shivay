...
const SYNC_URL="https://script.google.com/macros/s/AKfycbz12A9qSGQIIVspZRM9p5PpKuVPElVS244o2i5bXQQW-Aqo3NvTWb5VMszXOYa0OYzIsQ/exec";
const PROFILE_KEY="om_namah_shivay_profile_v1";
...
function loadProfile(){
 try{const p=JSON.parse(localStorage.getItem(PROFILE_KEY)); if(p&&p.userId&&p.name) return {userId:String(p.userId),name:String(p.name)};}catch(e){}
 const userId="user-"+cryptoRandomId();
 return {userId,name:""};
}
function cryptoRandomId(){try{if(window.crypto?.randomUUID)return window.crypto.randomUUID();}catch(e){}return Date.now().toString(36)+Math.random().toString(36).slice(2,10)}
...
async function syncRow(date,jaap,totalJaap){
 if(!profile.name || !SYNC_URL || !jaap)return;
 try{await fetch(SYNC_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({userId:profile.userId,name:profile.name,date,jaap:Number(jaap),totalJaap:Number(totalJaap),target:Number(data.targets.main||TARGET)})});}
 catch(e){console.warn("Google Sheet sync failed",e)}
}
async function migrateOldDataToSheet(){
 if(!profile.name)return;
 const marker="om_namah_shivay_migrated_v1_"+profile.userId;
 if(localStorage.getItem(marker))return;
 const entries=Object.entries(data.days).filter(([,v])=>Number(v)>0).sort(([a],[b])=>a.localeCompare(b));
 for(const [date,amount] of entries){await syncRow(date,Number(amount),Object.entries(data.days).filter(([d])=>d<=date).reduce((sum,[,v])=>sum+Number(v||0),0));}
 localStorage.setItem(marker,new Date().toISOString());
 showToast(entries.length?`જૂનો ${entries.length} દિવસનો Jaap Google Sheetમાં sync થયો. 🙏`:"Google Sheet sync તૈયાર છે. 🙏");
}
function syncAfterSave(date,amount){if(profile.name) syncRow(date,amount,total());}
...
$("#changeName").onclick=()=>{if(askForName(true))migrateOldDataToSheet()};
